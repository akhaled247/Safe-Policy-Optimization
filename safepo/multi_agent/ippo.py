# IPPO for SafePO multi-agent (classic IPPO local critic).
...
class IPPO_Policy:
    ...
    def __init__(self, config, obs_space, cent_obs_space, act_space):
        # Classic IPPO (arXiv:2011.09533): critic uses local obs only.
        self.config = config
        ...
        self.critic = Critic(config, self.obs_space, self.config["device"])
    ...
    def __init__(self,
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.share_policy and shared is not None:
                ...
                po = IPPO_Policy(config, obs_space, obs_space, act_space)
        ...
        self.buffer = []
        for agent_id in range(self.num_agents):
            tr = IPPO_Trainer(config, self.policy[agent_id])
            obs_space = self.envs.observation_space[agent_id]
...
if __name__ == '__main__':
    set_np_formatting()
    args, cfg_env, cfg_train = multi_agent_args(algo="ippo")
