# IPPO_LAG for SafePO multi-agent (classic IPPO local critic).
# Reference: arXiv:2011.09533; structure follows SafePO MAPPO family.
# share_policy default True (configurable).
...
class IPPO_L_Policy:
    ...
    def __init__(self, config, obs_space, cent_obs_space, act_space):
        # Classic IPPO-Lag: local reward/cost critics (arXiv:2011.09533 + SafePO Lag).
        self.config = config
        ...
        self.critic = Critic(config, obs_space, self.config["device"])
        self.cost_critic = Critic(config, obs_space, self.config["device"])
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        if rnn_states_cost is None:
            ...
            cost_preds, rnn_states_cost = self.cost_critic(obs, rnn_states_cost, masks)
            return values, actions, action_log_probs, rnn_states_actor, rnn_states_critic, cost_preds, rnn_states_cost

    def get_values(self, cent_obs, rnn_states_critic, masks):
        # IPPO: buffer share_obs slot holds local obs
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, action, masks,
        ...
        if rnn_states_cost is None:
            ...
            cost_values, _ = self.cost_critic(obs, rnn_states_cost, masks)
...
class Runner:
    ...
    def __init__(self,
        ...
        self.share_policy = bool(config.get("share_policy", True))
        self.policy = []
        shared = None
        for agent_id in range(self.num_agents):
            obs_space = self.envs.observation_space[agent_id]
            act_space = self.envs.action_space[agent_id]
            if self.share_policy and shared is not None:
                po = shared
            else:
                po = IPPO_L_Policy(config, obs_space, obs_space, act_space)
        ...
        self.buffer = []
        for agent_id in range(self.num_agents):
            tr = IPPO_L_Trainer(config, self.policy[agent_id])
            obs_space = self.envs.observation_space[agent_id]
    ...
    def warmup(self):
        ...
        for agent_id in range(self.num_agents):
            self.buffer[agent_id].share_obs[0].copy_(obs[:, agent_id])  # IPPO local
    ...
    def insert(self, data, aver_episode_costs=0):
        ...
        for agent_id in range(self.num_agents):
            if 'Frank'in self.config['env_name']:
                ...
                obs_to_insert = obs[:, agent_id]
            self.buffer[agent_id].insert(obs_to_insert, obs_to_insert, rnn_states[:, agent_id],
...
if __name__ == '__main__':
    set_np_formatting()
    args, cfg_env, cfg_train = multi_agent_args(algo="ippo_lag")
